...
* @version BETA 0.30
*/
$fp = fopen('process_id_parse_tweets.txt','w');
fwrite($fp,getmypid());
fclose($fp);
...
while (true) {
  ...
  while($row = mysqli_fetch_assoc($result)) {
    ...
    $news_image = $tweet_object->entities;
    $news_image_obj = $news_image->media;
    $news_image_url = $news_image_obj[0]->media_url;
    ...
    if ($oDB->in_table('users_streaming','user_id="' . $user_id . '"')) {
      $oDB->update('users_streaming',$field_values,'user_id = "' .$user_id . '"');
    } else {			
      $oDB->insert('users_streaming',$field_values);
    }
